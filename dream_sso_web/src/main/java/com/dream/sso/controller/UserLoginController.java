...
public class UserLoginController {
    ...
    public DreamResult login(String username, String password
        ...
        //如果登录成功，需要吧token加入cookie中 返回给浏览器
        if (result.getStatus()==200){
            //把token加入到cookie：DREAM_TOKEN_KEY=result.getData().toString()
    ...
    public String getUserByToken(@PathVariable String token,String callback) throws IOException {
        ...
        if (StringUtils.isNotBlank(callback)){//有callback代表json跨域请求
            //返回一个callback恒成立--也是一个json
